...
createMenu(name, settings){
    ...
    menus.name.icons := menus.dir "Programs\Images\"
    menus.name.shortcuts := menus.dir "Programs\Shortcut\"
    menus.name.settings := menus.dir "Programs\Settings\"

    menus.name.config := menus.name.settings settings
    ...
    runApp(title, exe, win, pth, runs, code, *) {
        ; runString := MsgBox("Hello")
            ...
            ;     Run app_path
                ...
                Try
                    Run menus.name.shortcuts title ".lnk"
                catch as e
