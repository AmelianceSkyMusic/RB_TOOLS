#include ..\..\RB [FUNCTION] Functions.ahk
    ...
    ;~ arr_Plugin := arr_Plugin_arg.arr_Plugin
        ...
        loop arr_Plugin.Length/6{
            ; Say arr_Plugin[1+n] " " j " " arr_Plugin[3+n] " " arr_Plugin[4+n] " " arr_Plugin[5+n] " " arr_Plugin[6+n] " " ImageListID
            ; obj_main[arr_Plugin[3+n]].Push := j
            trr := {
                        opacity: 3,
                        y: 300
                    }
...
; переменная_с_масивом = функция_получения_из_файла()
; ;категория, название, номер, путь к картинке

; объект = {}
; нмр := 0

; луп переменная_с_масивом.длинна/4 {

;     ПСМ = переменная_с_масивом
;     обект.ПСМ[нмр+1].ПСМ[нмр+2] = {ПСМ[нмр+3].ПСМ[нмр+4]}
...
;     объект = {
;         Категория1 = {
;             Название = {номер, путь к картинке}
;             Название = {номер, путь к картинке}
;             Название = {номер, путь к картинке}
;             Название = {номер, путь к картинке}
;         }
...
; HotIf ;numpad <caps-1234qwerasdfzxcv>
; for k,v in Map(StrSplit("1 div 2 mult 3 sub 4 add q 7 w 8 e 9 r enter a 4 s 5 d 6 f dot z 1 x 2 c 3 v 0", " ")*)
;   Hotkey "CapsLock & " k, ((v,*) => (mod := mods(), (key("{Blind}{Numpad" v "}"), say(modsVerbose(mod) v)) )).Bind(v)
...
; ai.nudge := {
;      up:{ hint:["↑", "↑.1", "↑.01"], blind:["", "^", "!^"], key:[   "{Up}", "^{F2}", "+^{F2}"] },
;    down:{ hint:["↓", "↓.1", "↓.01"], blind:["", "^", "!^"], key:[ "{Down}", "^{F3}", "+^{F3}"] },
;    left:{ hint:["←", "←.1", "←.01"], blind:["", "^", "!^"], key:[ "{Left}", "^{F5}", "+^{F5}"] },
;   right:{ hint:["→", "→.1", "→.01"], blind:["", "^", "!^"], key:["{Right}", "^{F7}", "+^{F7}"] },
; }
