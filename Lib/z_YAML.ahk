...
Yaml(& TextFileObject,Yaml:=0){
  ...
  ;~ G(p,& LF:=""){ ; get line and advance pointer to next line
  ...
  ;~ }
  L(& txt,Y:=0){ ; convert yaml to object
  ...
  U(& S,e:=1){ ; UniChar: convert unicode and special characters
    local v:=""
  ...
  C(& S){ ; CharUni: convert text to unicode notation
    local v:=""
  ...
  E(& S, J:=1){ ; EscIfNeed: check if escaping needed and convert to unicode notation
    If S=""
      return '""'
  ...
  S(& s, x){ ; Convert Spaces to level, 1 = first level = no spaces
    i:=0
    if Ord(s)=9
      x//=2
    Loop Parse, s
  ...
  J(& S,Y){ ; PureJSON: convert pure JSON Object
