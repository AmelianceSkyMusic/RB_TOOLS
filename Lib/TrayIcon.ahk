...
TrayIcon_GetHotItem() {
  idxTB := TrayIcon_GetTrayBar()
  res := SendMessage(TB_GETHOTITEM := 0x0447, 0, 0, "ToolbarWindow32" idxTB, "ahk_class Shell_TrayWnd")
  return res << 32 >> 32
}
...
TrayIcon_Button(sExeName, sButton := "L", bDouble := false, nIdx := 1) {
  d := A_DetectHiddenWindows
  DetectHiddenWindows(1)
  actions := Map( "WM_MOUSEMOVE"      , 0x0200
  ...
  if bDouble
    PostMessage(oIcons[nIdx]["msgid"], oIcons[nIdx]["uid"], actions[sButton "DBLCLK"],, "ahk_id " oIcons[nIdx]["hwnd"])
  else {
    PostMessage(oIcons[nIdx]["msgid"], oIcons[nIdx]["uid"], actions[sButton "DOWN"],, "ahk_id " oIcons[nIdx]["hwnd"])
    PostMessage(oIcons[nIdx]["msgid"], oIcons[nIdx]["uid"], actions[sButton "UP"],, "ahk_id " oIcons[nIdx]["hwnd"])
  }
  DetectHiddenWindows(d)
  return
}